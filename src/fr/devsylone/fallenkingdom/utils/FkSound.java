...
public enum FkSound
{
	...
	static{
		for(FkSound fSound:values()){
			try{
				fSound.setSound(Sound.valueOf(fSound.pre19sound));
			}catch(IllegalArgumentException e){
				try{
					fSound.setSound(Sound.valueOf(fSound.post19sound));
				}catch(IllegalArgumentException e2){}
			}
		}
		ArrayList<FkSound> notResolved=new ArrayList<FkSound>();
		
		for(FkSound fSound:values()){
			if(fSound.isResolved()){
