...
public enum FkSound
{
	...
	FkSound(String pre19sound, String post19sound)
	{
		this(pre19sound, post19sound, post19sound);
	}

	FkSound(String pre19sound, String post19sound, String post113sound)
	{
		this.pre19sound = pre19sound;
		this.post19sound = post19sound;
		this.post113sound = post113sound;
	}
	...
	public boolean isResolved(){
		return resolvedSound != null;
	}
	
	public int getScore(Sound other){
		if(name().equals(other.name()))
			return 20;
		String[] myParts=post19sound.split("_");
		String[] otherParts=other.name().split("_");
		int score=0;
		for(String myPart:myParts){
			for(String oPart:otherParts){
				if(myPart.equals(oPart)){
					score++;
					break;
				}
			}
	...
	static{
		ArrayList<FkSound> notResolved = new ArrayList<>();
		...
		for (FkSound fSound : values()) {
			...
			if (s != null) {
				...
				notResolved.add(fSound);
			}
		}
		
		while(!notResolved.isEmpty()){
			int maxScore=0;
			FkSound maxFSound = null;
			Sound maxSound = null;
			
			for(FkSound fSound:notResolved){
				int currentMaxScore=0;
				Sound currentMaxSound = null;
				
				for(Sound sound:Sound.values()){
					int currentScore=fSound.getScore(sound);
					if(currentScore>currentMaxScore){
						currentMaxSound=sound;
